...
export async function listCutedogClipIds(): Promise<readonly string[] | undefined> {
  try {
    const html = await (await fetch(url)).text();
    const matches = html.matchAll(clipPattern);
    return Array.from(matches, (m: ReadonlyRegExpExecArray) => m[1]);
  } catch (error: unknown) {
    console.error(`Error at listCutedogClipIds: ${error instanceof Error ? error : 'error'}`);
    return undefined;
  }
}
